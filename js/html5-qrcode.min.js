...
qrcode = {
    ...
    callback: null,
    decode: function(d) {
        /* if (0 == arguments.length) {                      // gets live image from HTML5 canvas if there is no specified input
        ...
        b.onload = function() {
            var a = document.createElement("canvas"),
                c = a.getContext("2d"),
                d = document.getElementById("out-canvas"); // seems to be null in all cases... built in for developer's use case?
            null != d && (d = d.getContext("2d"), d.clearRect(0, 0, 320, 240), d.drawImage(b, 0, 0, 320, 240));
            a.width = b.width;
            a.height = b.height;
            c.drawImage(b, 0, 0); // draws the uploaded image into the created canvas
            qrcode.width = b.width;
            qrcode.height = b.height;
            try {
                qrcode.imagedata = c.getImageData(0, 0, b.width, b.height) // saves qrcode imagedata inherited from the newly created canvas (inheriting height and width from uploaded pic)
            } catch (h) {
            ...
            try {
                qrcode.result = qrcode.process(c); // passes the canvas containing the uploaded image to the process function
            } catch (j) {
                console.log(j), qrcode.result = j;
            }
            null != qrcode.callback && qrcode.callback(qrcode.result)
        };
        b.src = d; // sets the source url of the image to be the uploaded blob
    },
    decode_utf8: function(d) {
        return decodeURIComponent(escape(d))
    },
    process: function(d) {
        var a = (new Date).getTime(), // register beginning timestamp of decode function
            c = qrcode.grayScaleToBitmap(qrcode.grayscale()); // nested functions to process image; first, turn image to grayscale, then turn grayscale to bitmap
        if (qrcode.debug) {
...
(function(d) {
    d.fn.html5_qrcode = function(a, c, b) {
        /*  var e = this.height(),            // This is all used to deal with the getUserMedia function (live detection)
            g = this.width();
        null == e && (e = 250);
        null == g && (g = 300);
